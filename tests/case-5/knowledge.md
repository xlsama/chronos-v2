...
1. `data_sources` 先完成原始数据同步
2. `scheduled_jobs.generate_daily_report` 在凌晨执行聚合
3. 聚合结果写入 `daily_reports`
4. `dashboard-api` 和 `bi-gateway` 读取 `daily_reports` 展示给业务方

因此，若 `data_sources` 正常同步但 `daily_reports` 停留在旧日期，优先检查 `generate_daily_report`，而不是先怀疑数据源连接故障。
...
- 先看 `daily_reports` 的最新 `report_date`
- 再看 `scheduled_jobs` 中 `generate_daily_report` 的 `is_enabled`、`status`、`last_run_at`
- 然后用 `app_errors` 校验禁用是否已经连续多天发生
- 最后用 `data_sources` 排除“源数据没同步”这一类假设
