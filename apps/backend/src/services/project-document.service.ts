...
type DocumentIndexingReason = 'empty_content' | 'empty_chunks' | 'image_without_ocr' | 'index_write_skipped'
...
const activeDocumentProcesses = new Map<string, { controller: AbortController; promise: Promise<unknown>; runId: symbol }>()
...
function runTrackedDocumentProcess<T>(documentId: string, work: (signal: AbortSignal) => Promise<T>) {
...
export const projectDocumentService = {
  ...
  async listPaginated(
    projectId: string,
    options: { kind?: DocumentKind; publicationStatus?: PublicationStatus; page?: number; pageSize?: number } = {},
  ) {
    const conditions = [eq(projectDocuments.projectId, projectId)]
    if (options.kind) conditions.push(eq(projectDocuments.kind, options.kind))
    if (options.publicationStatus) conditions.push(eq(projectDocuments.publicationStatus, options.publicationStatus))
    ...
    const [rows, [{ total }]] = await Promise.all([
      db.select().from(projectDocuments).where(where).orderBy(desc(projectDocuments.createdAt)).limit(pageSize).offset(offset),
  ...
  async deleteDocument(id: string) {
    ...
    await db.update(projectDocuments).set({ status: 'cancelled' }).where(eq(projectDocuments.id, id))
    ...
    return runTrackedDocumentProcess(documentId, async (signal) => {
      try {
        ...
        const embeddings = chunks.length > 0
          ? await embedTexts(chunks.map((chunk) => chunk.content), { signal })
          : []
        throwIfDocumentProcessAborted(signal)
        ...
        const indexingState = chunks.length > 0
        ...
        if (error instanceof DocumentProcessAbortedError || (error instanceof Error && error.name === 'DocumentProcessAbortedError')) {
          return null
        }
        ...
        await db.update(projectDocuments).set({
          ...
          parserError: message,
          embeddingModel: null,
          metadata: withIndexingMetadata(currentDocument?.metadata, buildNotIndexedState({
  ...
  async search(query: string, options: {
    ...
    const documents = documentIds.length > 0
      ? await db.select().from(projectDocuments).where(and(
...
async function getDocumentState(documentId: string) {
  ...
  }).from(projectDocuments).where(eq(projectDocuments.id, documentId))
...
function deriveIndexingState(row: {
  ...
  if (isDocumentIndexingStatus(metadataStatus)) {
    return {
      ...
      vectorCount: typeof metadataVectorCount === 'number' && Number.isFinite(metadataVectorCount)
        ? metadataVectorCount
...
function withPublicUrl<T extends {
  filePath: string
  metadata?: unknown
  content?: string | null
  chunkCount: number
