...
async function processAgentJob(job: Job<AgentBackgroundJobData>) {
  const { threadId, incidentId, content, projectId, summary } = job.data
  const incident = { id: incidentId, content, projectId, summary }

  const controller = new AbortController()
  registerActiveAgent(threadId, controller)
  ...
  try {
    ...
    logger.info({ threadId, incidentId: incident.id, jobId: job.id }, '[Agent] supervisor started (background via queue)')
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
      ...
      try {
        if (attempt > 1) {
          logger.warn(
            { threadId, incidentId: incident.id, attempt, maxAttempts: MAX_ATTEMPTS },
        ...
        if (timeoutSignal.aborted) {
          throw new Error(`Background diagnosis attempt timed out after ${ATTEMPT_TIMEOUT_MS}ms`)
        }
    ...
    removeActiveAgent(threadId)
  }
}

export function createAgentBackgroundWorker() {
  const worker = new Worker<AgentBackgroundJobData>('agent-background', processAgentJob, {
    connection: redisConnection,
    concurrency: 1,
    lockDuration: 600_000, // 10 min for tool approval waits
  })

  worker.on('completed', (job) => {
    logger.info({ jobId: job.id, threadId: job.data.threadId }, '[Queue] agent-background job completed')
  })

  worker.on('failed', (job, err) => {
    logger.error({ jobId: job?.id, threadId: job?.data.threadId, err }, '[Queue] agent-background job failed')
  })

  return worker
}

// --- Helper functions (moved from agent-runner.ts) ---
